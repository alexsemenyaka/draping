...
def func_contains_sub(): pass

@pytest.fixture
def funcs_list():
    return [TestClass.add_user, TestClass.pay_bill, TestClass.report_error, func_contains_sub]
...
class TestHelpers:
    ...
    def test_contains(self, funcs_list):
        assert contains(TestClass, "add", "error") == (TestClass.add_user, TestClass.cls_add, TestClass.report_error)
        assert contains(funcs_list, "sub") == (func_contains_sub,)
        assert contains(TestClass) == ()  # No substrings: empty

    def test_not_contains(self, funcs_list):
        assert not_contains(TestClass, "add", "pay") == (TestClass._private, TestClass.report_error)
        assert not_contains(funcs_list, "error") == (TestClass.add_user, TestClass.pay_bill, func_contains_sub)
        assert not_contains(TestClass) == (TestClass._private, TestClass.add_user, TestClass.cls_add, TestClass.pay_bill, TestClass.report_error, TestClass.stat_pay)  # No substrings: all

    def test_positive_re(self, funcs_list):
        assert positive_re(TestClass, r'^add', r'error$') == (TestClass.add_user, TestClass.report_error)
        assert positive_re(funcs_list, r'con.*sub') == (func_contains_sub,)
    ...
    def test_negative_re(self, funcs_list):
        ...
        assert negative_re(funcs_list, r'error$') == (TestClass.add_user, TestClass.pay_bill, func_contains_sub)
